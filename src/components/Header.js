...
// Header component
/* Dark Mode Management ***: The component uses useState and useEffect to toggle dark mode, which is stored in localStorage and applied to the document element.
Navigation ***: The component provides both mobile and desktop navigation options, including a dynamic search form and navigation links.
Responsive Design*** : The component uses Tailwind CSS utility classes for responsiveness and layout. The mobile menu is toggled using the hidden state variable.
Active Navigation Link Styling ***: The navigation links change styles depending on whether they are active using NavLink and dynamic classes. */

export const Header = () => {
  // State to manage the visibility of the mobile menu
  const [hidden, setHidden] = useState(true);

  // State to manage the dark mode, loaded from localStorage if available
  const [darkMode, setDarkMode] = useState( JSON.parse(localStorage.getItem("darkMode")) || false);

  // useNavigate hook to handle programmatic navigation
  const navigate = useNavigate();

  // Effect hook to update localStorage and apply dark mode class to document element
  useEffect(() => {
    // Save dark mode setting to localStorage
    localStorage.setItem("darkMode", JSON.stringify(darkMode));

    // Apply or remove dark class from document root based on the darkMode state
    if(darkMode){
  ...
  // CSS classes for active and inactive navigation links
  ...
  // Handle the search form submission, navigate to search results
  const handleSubmit = (event) => {
    event.preventDefault();
    const queryTerm = event.target.search.value; // Get the search term from the input
    event.target.reset(); // Reset the input field after submission
    return navigate(`/search?q=${queryTerm}`); // Navigate to the search page with the query
  }
    ...
    <header>
      ...
      <nav className="bg-white border-b-2 border-gray-200 px-2 sm:px-4 py-2 dark:bg-gray-900 dark:border-b-1 dark:border-gray-900">
        <div className="flex flex-wrap justify-between items-center mx-auto">
          ...
          {/* Mobile navigation and dark mode toggle */}
          <div id="mobile-nav" className="flex md:order-2">
            {/* Dark mode toggle button */}
            <button onClick={() => setDarkMode(!darkMode)} data-tooltip-target="navbar-search-example-toggle-dark-mode-tooltip" type="button" className="flex items-center p-2 mr-2 text-xs font-medium text-gray-700 bg-white rounded-lg border border-gray-200 toggle-dark-state-example hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-500 dark:bg-gray-800 focus:outline-none dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
